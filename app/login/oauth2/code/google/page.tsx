...
export default function GoogleCallbackPage() {
  ...
  const handleCallback = async () => {
    try {
      ...
      console.log("[v0] Google OAuth callback from backend:", {
        callbackStatus,
      ...
      if (error) {
        setStatus("error")
        setErrorMessage(errorDescription || "구글 로그인이 취소되었습니다")
        return
      }
      ...
      if (callbackStatus === "signup_required" && signupToken) {
        ...
        router.push(`/oauth2/signup?provider=google&token=${signupToken}`)
        return
      }
