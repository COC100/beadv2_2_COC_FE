...
export default function NaverCallbackPage() {
  ...
  const handleCallback = async () => {
    try {
      ...
      console.log("[v0] Naver OAuth callback:", { callbackStatus, token, error, errorDescription })
      ...
      if (callbackStatus === "signup_required" && token) {
        ...
        router.push(`/oauth2/signup?provider=naver&token=${token}`)
        return
      }
      ...
      console.error("[v0] Naver OAuth callback error:", error)
      setStatus("error")
