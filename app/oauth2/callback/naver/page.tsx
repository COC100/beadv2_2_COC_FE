...
export default function NaverCallbackPage() {
  ...
  useEffect(() => {
    ...
    console.log("[v0] Naver callback:", { code, state, error, errorDescription })
  ...
  const handleNaverLogin = async (code: string) => {
    try {
      ...
      // 신규 회원인 경우 - 추가 정보 입력 페이지로 이동
        ...
        router.push(`/oauth2/signup?provider=naver&token=${data.signupToken}`)
      } else {
        throw new Error("예상치 못한 응답입니다")
      }
    } catch (error: any) {
      console.error("[v0] Naver login error:", error)
      setStatus("error")
